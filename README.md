...
| `apps/mobile` | Expo(React Native) 앱 — *아직 없음, `docs/mobile-rn-plan.md` Phase 1에서 생성* |
...
```bash
pnpm install
cp apps/web/.env.local.example apps/web/.env.local   # NEXT_PUBLIC_API_BASE_URL 설정
pnpm dev
```

[http://localhost:3000](http://localhost:3000) — 백엔드(`../memento`)를 `./gradlew bootRun`으로 함께 띄워야 데이터가 보인다.
