...
interface AuthState {
  member: Member | null;
  /** False until SecureStore has been read — "we don't know yet", not "logged out". */
  isHydrated: boolean;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Unlike the web, this genuinely is asynchronous: SecureStore has no synchronous read,
  // so until it settles the app must show neither the shell nor the login screen. The
  // root layout keeps the splash up on this flag.
  useEffect(() => {
    let cancelled = false;
    hydrateTokens().then(() => {
