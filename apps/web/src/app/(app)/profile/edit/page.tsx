...
import { membersApi, toErrorMessage } from "@/lib/core";
...
function EditProfileForm({ member }: { member: Member }) {
  ...
  const handleError = (err: unknown, fallback: string) => {
    showToast(toErrorMessage(err, fallback));
  };

  const avatarMutation = useMutation({
    mutationFn: (file: File) => membersApi.updateMyProfileImage(file),
  ...
  const saveMutation = useMutation({
    mutationFn: () => membersApi.updateMe({ nickname, bio }),
    onSuccess: () => {
