import { memoriesApi } from "@/lib/core";
...
// memoriesApi.getMemory() runs unauthenticated here (server-side request() has no window,
...
export async function generateMetadata({ params }: PageProps<"/entry/[id]">): Promise<Metadata> {
  ...
  try {
    const memory = await memoriesApi.getMemory(id);
