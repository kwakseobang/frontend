...
async function toMemoryForm(input: MemoryWriteInput): Promise<FormData> {
  const { formData } = getCoreConfig();
  const form = new FormData();
  const payload = { content: input.content, memoryAt: input.memoryAt, visibility: input.visibility };
  await formData.appendJsonPart(form, "request", payload);
  if (input.images) {
    for (const image of input.images) formData.appendFilePart(form, "files", image);
  }
  return form;
}

export async function createMemory(input: MemoryWriteInput) {
  return request<number>("/api/memories", { method: "POST", form: await toMemoryForm(input) });
}

export async function updateMemory(id: number | string, input: MemoryWriteInput) {
  return request<void>(`/api/memories/${id}`, { method: "PATCH", form: await toMemoryForm(input) });
}
...
export async function createDraft(input: MemoryDraftInput) {
  return request<number>("/api/memories/drafts", { method: "POST", form: await toMemoryForm(input) });
}
